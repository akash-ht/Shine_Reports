...
select details.order_id as Order_Id,
     ...
     details.Combo_Name as Combo_Name,
     details.Payment_mode,
     if(details.CandidateName is null or details.CandidateName='', 'NotAvailable',details.CandidateName) as Candidate_Name,
     if(details.CandidateEmail is null or details.CandidateEmail='', 'NotAvailable',details.CandidateEmail) as Candidate_Email,
...
from
         (
             ...
             left join shinecp.cart_productvariation as combov
             on combov.id = coi.combo_id
...
left join
         (
             select oi.order_id,
                 ...
                 IFNULL(ed.Coupon_Discount,'0') as CouponDiscount, if(ed.NetDiscount=0,'0',round((1-pow(((100-ed.NetDiscount)/100),1/(oi.Items_Count)))*100,0))as DistributedDiscount
             from
             (
                 ...
                 left join shinecp.cart_order as co
                 on coi.order_id=co.id
                 #where date(co.added_on)>=date_add(curdate(), interval-1 day)
             ...
             (
                 ...
                 select co.id as order_id,
                     ...
                     if(op.GrossPrice_Order-co.amount_payable=0,'NoDiscountOffered',round(((op.GrossPrice_Order-co.amount_payable)/op.GrossPrice_Order)*100,0))as NetDiscount,
                     co.coupon_id,
